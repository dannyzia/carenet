...
export default function IncidentReportPage() {
  ...
  useDocumentTitle(tDocTitle("pageTitles.incidentReport", "Incident Report"));

  const { data: patients, loading } = useAsyncData(() => caregiverService.getAssignedPatients());
  const { data: history, loading: l2 } = useAsyncData(() => caregiverService.getIncidentHistory());
...
function IncidentContent({ patients, history }: { patients: any[]; history: any[] }) {
  ...
  const handleSubmit = async () => {
    await caregiverService.reportIncident({
      ...
      <div className="max-w-lg mx-auto space-y-5">
        <div className="finance-card p-8 text-center space-y-4">
          ...
          <button onClick={() => { setSubmitted(false); setMode("list"); setStep(0); setForm({ type: "", severity: "", patientId: "", description: "", immediateAction: "", photos: [] }); }}
            className="px-6 py-2.5 rounded-xl text-white text-sm" style={{ background: "var(--cn-gradient-caregiver)" }}>
            Back to Incidents
          </button>
        </div>
      </div>
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { setMode("form"); setStep(0); }} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-white text-sm" style={{ background: "var(--cn-gradient-caregiver)" }}>
    ...
    <div className="max-w-lg mx-auto space-y-5">
      ...
      {step === 1 && (
        <div className="space-y-4">
          ...
          <button onClick={() => setStep(2)} disabled={!form.severity || !form.patientId}
            className="w-full py-3 rounded-xl text-white text-sm disabled:opacity-50" style={{ background: "var(--cn-gradient-caregiver)" }}>
            Continue
          </button>
        </div>
      )}
      ...
      {step === 3 && (
        <div className="space-y-4">
          ...
          <button onClick={handleSubmit} className="w-full py-3 rounded-xl text-white text-sm" style={{ background: "var(--cn-gradient-caregiver)" }}>
