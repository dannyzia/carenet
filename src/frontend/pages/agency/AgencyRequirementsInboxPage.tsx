  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">{[{ label: "New This Week", value: String(newCount), icon: Inbox, color: "#0288D1", bg: "rgba(2,136,209,0.12)" }, { label: "Avg Response", value: "6.2 hrs", icon: Timer, color: cn.teal, bg: cn.tealBg }, { label: "Conversion Rate", value: "72%", icon: TrendingUp, color: cn.green, bg: cn.greenBg }].map((s) => { const Icon = s.icon; return (<div key={s.label} className="stat-card"><div className="w-9 h-9 rounded-lg flex items-center justify-center mb-2" style={{ background: s.bg }}><Icon className="w-4 h-4" style={{ color: s.color }} /></div><p className="text-lg" style={{ color: cn.text }}>{s.value}</p><p className="text-xs" style={{ color: cn.textSecondary }}>{s.label}</p></div>); })}</div>
