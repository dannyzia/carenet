  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">{[{ label: "On-Time Rate", value: "92%", color: cn.green, bg: cn.greenBg, icon: CheckCircle2 }, { label: "Active Shifts", value: String(activeShifts.length), color: cn.teal, bg: cn.tealBg, icon: Users }, { label: "Upcoming Today", value: "6", color: cn.blue, bg: cn.blueBg, icon: Clock }, { label: "Incidents Today", value: "0", color: cn.amber, bg: cn.amberBg, icon: AlertTriangle }].map((s) => { const Icon = s.icon; return (<div key={s.label} className="stat-card"><Icon className="w-5 h-5 mb-2" style={{ color: s.color }} /><p className="text-lg" style={{ color: s.color }}>{s.value}</p><p className="text-xs" style={{ color: cn.textSecondary }}>{s.label}</p></div>); })}</div>
