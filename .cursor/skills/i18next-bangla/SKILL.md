...
Follow these rules:
- After adding or changing `t()` / `Trans` keys in code, run **`npm run i18n:extract`** (merges keys into `src/locales/en/*.json`), then **`npm run i18n:sync`** or rely on the Vite `vite-i18n-sync` plugin on dev/build. Use **`npm run i18n:refresh`** to run both.
