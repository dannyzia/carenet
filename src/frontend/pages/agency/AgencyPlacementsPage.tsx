  ...
  return (
    <>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">{[{ label: "Active", value: "8", color: cn.green, bg: cn.greenBg }, { label: "Avg Duration", value: "2.3 mo", color: cn.teal, bg: cn.tealBg }, { label: "Shifts This Week", value: "42", color: cn.blue, bg: cn.blueBg }, { label: "Utilization", value: "87%", color: cn.amber, bg: cn.amberBg }].map((s) => (<div key={s.label} className="stat-card"><p className="text-lg" style={{ color: s.color }}>{s.value}</p><p className="text-xs" style={{ color: cn.textSecondary }}>{s.label}</p></div>))}</div>
