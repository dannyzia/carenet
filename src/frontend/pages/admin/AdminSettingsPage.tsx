...
export default function AdminSettingsPage() {
  ...
  const Toggle = ({ settingKey }: { settingKey: string }) => (<button onClick={() => toggle(settingKey)} className="relative w-12 h-6 rounded-full transition-all" style={{ background: settings && settings[settingKey as keyof typeof settings] ? "#5FB865" : "#E5E7EB" }}><span className="absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-all" style={{ left: settings && settings[settingKey as keyof typeof settings] ? "calc(100% - 22px)" : "2px" }} /></button>);
