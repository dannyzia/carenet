        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1 space-y-6">
            <div className="finance-card p-6 text-center">
              ...
              <Link to={`/guardian/agency/${caregiver.agency.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs mb-4" style={{ background: cn.tealBg, color: cn.teal }}><Building2 className="w-3 h-3" />Works with: {caregiver.agency.name}</Link>
            ...
            <div className="flex flex-col gap-3">
              <Link to={`/guardian/care-requirement-wizard?agency=${caregiver.agency.id}`}><Button className="w-full h-14 rounded-2xl shadow-lg text-lg font-bold" style={{ background: "var(--cn-gradient-guardian)" }}>Submit Care Requirement</Button></Link>
              <Link to={`/guardian/agency/${caregiver.agency.id}`}><Button variant="outline" className="w-full h-12 rounded-xl" style={{ borderColor: cn.teal, color: cn.teal }}><Building2 className="w-5 h-5 mr-2" /> Contact Agency</Button></Link>
            </div>
          </div>
