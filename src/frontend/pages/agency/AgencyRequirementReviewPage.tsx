...
import { ChevronLeft, User, Heart, Calendar, Clock, MapPin, Send, Star, CheckCircle2, Search, AlertCircle, FileText } from "lucide-react";
...
export default function AgencyRequirementReviewPage() {
  ...
  const [proposedRate, setProposedRate] = useState("2800");
  ...
  return (
    <>
      <div className="max-w-3xl mx-auto space-y-6 pb-8">
        ...
        <div className="finance-card p-5 space-y-3"><h3 className="text-sm" style={{ color: cn.text }}>Requirement Details</h3><div className="grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs" style={{ color: cn.textSecondary }}>Guardian</p><p className="flex items-center gap-1" style={{ color: cn.text }}>Rashed Hossain <CheckCircle2 className="w-3 h-3" style={{ color: cn.green }} /></p></div><div><p className="text-xs" style={{ color: cn.textSecondary }}>Patient</p><p style={{ color: cn.text }}>Mr. Abdul Rahman, 74y</p></div><div><p className="text-xs" style={{ color: cn.textSecondary }}>Care Type</p><p style={{ color: cn.text }}>Full Day Care</p></div><div><p className="text-xs" style={{ color: cn.textSecondary }}>Duration</p><p style={{ color: cn.text }}>3 Months</p></div><div><p className="text-xs" style={{ color: cn.textSecondary }}>Shift</p><p style={{ color: cn.text }}>Day (8AM-8PM)</p></div><div><p className="text-xs" style={{ color: cn.textSecondary }}>Budget</p><p style={{ color: cn.green }}>{"\u09F3"} 2,000 - {"\u09F3"} 3,500/day</p></div></div><div><p className="text-xs mb-1" style={{ color: cn.textSecondary }}>Special Requirements</p><p className="text-sm" style={{ color: cn.text }}>Patient needs assistance with daily mobility exercises. Prefers Bangla-speaking caregiver.</p></div></div>
        ...
        <div className="finance-card p-5 space-y-4"><h3 className="text-sm" style={{ color: cn.text }}>Proposal Details</h3><div className="grid grid-cols-2 gap-4"><div><label className="text-xs mb-1 block" style={{ color: cn.textSecondary }}>Proposed Rate (BDT/day)</label><div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm" style={{ color: cn.textSecondary }}>{"\u09F3"}</span><input type="number" value={proposedRate} onChange={(e) => setProposedRate(e.target.value)} className="w-full pl-8 pr-4 py-3 rounded-xl border text-sm" style={{ background: cn.bgInput, borderColor: cn.border, color: cn.text }} /></div></div><div><label className="text-xs mb-1 block" style={{ color: cn.textSecondary }}>Agency Margin</label><div className="px-4 py-3 rounded-xl text-sm" style={{ background: cn.bgInput, color: cn.green }}>{"\u09F3"} {Math.round(Number(proposedRate) * 0.2)}/day (20%)</div></div></div><div><label className="text-xs mb-1.5 block" style={{ color: cn.textSecondary }}>Notes for Guardian</label><textarea rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Any notes about the proposed care plan..." className="w-full px-4 py-3 rounded-xl border text-sm resize-none" style={{ background: cn.bgInput, borderColor: cn.border, color: cn.text }} /></div></div>
