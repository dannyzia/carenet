...
export const adminService = {
  async getDashboardData(): Promise<AdminDashboardData> {
    if (USE_SUPABASE) {
      return sbRead("admin:dashboard", async () => {
  ...
  async getPaymentsData(): Promise<AdminPaymentsData> {
    if (USE_SUPABASE) {
      return sbRead("admin:payments", async () => {
  ...
  async getVerifications(): Promise<AdminVerification[]> {
    if (USE_SUPABASE) {
      return sbRead("admin:verifications", async () => {
        ...
        return (data || []).map((d: any) => ({
          id: d.id,
          userName: d.caregiver_profiles?.name || "Unknown",
          userRole: "caregiver",
          documentType: d.category,
          documentName: d.name,
          status: d.status,
          submittedAt: d.uploaded || d.created_at,
        }));
      });
    }
    await delay();
  ...
  async getUsers(): Promise<AdminUser[]> {
    if (USE_SUPABASE) {
      return sbRead("admin:users", async () => {
        ...
        return (data || []).map((d: any) => ({
          ...
          role: d.role, status: d.status || "active",
          createdAt: d.created_at, lastLogin: d.last_sign_in_at,
        }));
      });
    }
    await delay();
  ...
  async getPlacements(): Promise<AdminPlacement[]> {
    if (USE_SUPABASE) {
      return sbRead("admin:placements", async () => {
  ...
  async getAgencyPerformance(): Promise<AgencyPerformanceRow[]> {
    if (USE_SUPABASE) {
      return sbRead("admin:agency-perf", async () => {
  ...
  async getAuditChartData(): Promise<AuditChartDataPoint[]> {
    if (USE_SUPABASE) {
      return sbRead("admin:audit-chart", async () => {
