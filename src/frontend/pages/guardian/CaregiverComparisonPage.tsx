      ...
      <div className="max-w-5xl mx-auto px-4 md:px-6 -mt-12 md:-mt-16">
        <div className="md:hidden">
          <div className="overflow-x-auto snap-x snap-mandatory flex gap-4 cn-scroll-x -mx-2 px-2 pb-4" onScroll={(e) => { const el = e.currentTarget; const idx = Math.round(el.scrollLeft / (el.scrollWidth / selectedCaregivers.length)); setMobileIndex(Math.min(idx, selectedCaregivers.length - 1)); }}>
            {selectedCaregivers.map((c) => (<div key={c.id} className="snap-center shrink-0 w-[85vw] max-w-[340px]"><div className="rounded-2xl overflow-hidden" style={{ background: cn.bgCard, boxShadow: cn.shadowCard }}><div className="p-5 text-center"><img src={c.img} className="w-16 h-16 rounded-2xl object-cover mx-auto ring-2 ring-white shadow-lg mb-3" alt={c.name} /><h3 className="text-base" style={{ color: cn.textHeading }}>{c.name}</h3><p className="text-xs uppercase tracking-widest mt-0.5" style={{ color: cn.pink }}>{c.type}</p><span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-lg mt-1" style={{ background: cn.tealBg, color: cn.teal }}>{c.agency.name}</span></div><div className="px-5 pb-3 space-y-3">{comparisonRows.map((row) => (<div key={row.key} className="flex items-center justify-between py-2" style={{ borderTop: `1px solid ${cn.borderLight}` }}><div className="flex items-center gap-2"><row.icon className={`w-4 h-4 ${row.color}`} /><span className="text-xs" style={{ color: cn.textSecondary }}>{row.label}</span></div><span className="text-sm" style={{ color: cn.text, fontWeight: 600 }}>{String(c[row.key as keyof typeof c])}</span></div>))}</div><div className="px-5 pb-5"><Link to={`/guardian/care-requirement-wizard?agency=${c.agency.id}`} className="block w-full"><Button className="w-full h-12 text-white text-xs rounded-xl" style={{ background: "var(--cn-gradient-guardian)" }}>Request via Agency</Button></Link></div></div></div>))}
          </div>
        </div>
        <div className="hidden md:block finance-card overflow-hidden">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">{comparisonRows.map((row, i) => (<tr key={i} className="hover:bg-gray-50/30 transition-all"><td className="p-8"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-lg bg-gray-50 flex items-center justify-center"><row.icon className={`w-4 h-4 ${row.color}`} /></div><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{row.label}</span></div></td>{selectedCaregivers.map(c => (<td key={c.id} className="p-8 text-center"><span className="text-sm font-black text-gray-800">{c[row.key as keyof typeof c]}</span></td>))}</tr>))}<tr><td className="p-8"></td>{selectedCaregivers.map(c => (<td key={c.id} className="p-8 text-center"><Link to={`/guardian/care-requirement-wizard?agency=${c.agency.id}`} className="block w-full"><Button className="w-full h-12 text-white font-black text-xs rounded-xl shadow-xl transition-all" style={{ background: "var(--cn-gradient-guardian)" }}>Request via Agency</Button></Link></td>))}</tr></tbody>
          </table>
        </div>
