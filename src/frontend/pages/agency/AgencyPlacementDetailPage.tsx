  ...
  return (
    <>
      <div className="max-w-3xl mx-auto space-y-6 pb-8">
        ...
        <div className="finance-card p-5"><h3 className="text-sm mb-3" style={{ color: cn.text }}>Assigned Caregivers</h3><div className="space-y-2">{[{ name: "Karim Uddin", shifts: 38, rate: "\u09F3 1,200/shift", status: "Primary" }, { name: "Fatema Akter", shifts: 4, rate: "\u09F3 1,400/shift", status: "Backup" }].map((cg) => (<div key={cg.name} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: cn.bgInput }}><div className="w-8 h-8 rounded-full flex items-center justify-center text-xs" style={{ background: cn.pinkBg, color: cn.pink }}>{cg.name.split(" ").map(w => w[0]).join("")}</div><div className="flex-1"><p className="text-sm" style={{ color: cn.text }}>{cg.name}</p><p className="text-xs" style={{ color: cn.textSecondary }}>{cg.shifts} shifts | {cg.rate}</p></div><span className="text-xs px-2 py-0.5 rounded-full" style={{ background: cg.status === "Primary" ? cn.tealBg : cn.amberBg, color: cg.status === "Primary" ? cn.teal : cn.amber }}>{cg.status}</span></div>))}<button className="w-full p-3 rounded-xl border-2 border-dashed text-sm flex items-center justify-center gap-2" style={{ borderColor: cn.border, color: cn.textSecondary }}><Plus className="w-4 h-4" /> Assign New Caregiver</button></div></div>
        <div className="finance-card p-5"><h3 className="text-sm mb-3" style={{ color: cn.text }}>Financial Summary</h3><div className="grid grid-cols-3 gap-4 text-center"><div><p className="text-xs" style={{ color: cn.textSecondary }}>Billed to Guardian</p><p className="text-lg" style={{ color: cn.text }}>{"\u09F3"} 75,600</p></div><div><p className="text-xs" style={{ color: cn.textSecondary }}>Paid to Caregivers</p><p className="text-lg" style={{ color: cn.pink }}>{"\u09F3"} 55,200</p></div><div><p className="text-xs" style={{ color: cn.textSecondary }}>Agency Margin</p><p className="text-lg" style={{ color: cn.green }}>{"\u09F3"} 20,400</p></div></div></div>
      </div>
    </>
  );
}