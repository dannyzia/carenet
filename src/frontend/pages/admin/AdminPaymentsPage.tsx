...
export default function AdminPaymentsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">{[{ label: "Revenue (Mar)", value: "\u09F3 2,89,000", icon: ArrowDownRight, color: "#5FB865", bg: "#7CE57720" }, { label: "Payouts (Mar)", value: "\u09F3 2,24,000", icon: ArrowUpRight, color: "#EF4444", bg: "#EF444420" }, { label: "Platform Fees", value: "\u09F3 14,450", icon: CreditCard, color: "#7B5EA7", bg: "#8082ED20" }, { label: "Pending Payouts", value: "\u09F3 12,800", icon: Clock, color: "#E8A838", bg: "#FFB54D20" }].map(s => { const Icon = s.icon; return (<div key={s.label} className="cn-stat-card"><div className="flex items-center justify-between mb-3"><div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: s.bg }}><Icon className="w-5 h-5" style={{ color: s.color }} /></div><TrendingUp className="w-4 h-4" style={{ color: cn.green }} /></div><p className="text-xl" style={{ color: cn.text }}>{s.value}</p><p className="text-xs mt-0.5" style={{ color: cn.textSecondary }}>{s.label}</p></div>); })}</div>
