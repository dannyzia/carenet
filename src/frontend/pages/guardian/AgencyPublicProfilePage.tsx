  ...
  return (
    <>
      <div className="max-w-3xl mx-auto space-y-6 pb-8">
        <Link to="/guardian/search" className="inline-flex items-center gap-1 text-sm" style={{ color: cn.textSecondary }}><ChevronLeft className="w-4 h-4" /> Back to Search</Link>
        ...
        <Link to={`/guardian/care-requirement-wizard?agency=${id || "a1"}`} className="block w-full py-3.5 rounded-xl text-white text-center text-sm" style={{ background: "var(--cn-gradient-guardian)" }}>Submit Care Requirement</Link>
      </div>
    </>
  );
}